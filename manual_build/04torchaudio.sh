...
# =============================== 需要构建的版本
TO_BUILD_VERSIONS=("2.8.0" "2.7.1" "2.7.0" "2.6.0" "2.5.1" "2.5.0" "2.4.1")

# =============================== 构建变量
export PKG_NAME="torchaudio"
export FFMPEG_ROOT=/opt/ext/ffmpeg/ffmpeg-n5.0.3
export LD_LIBRARY_PATH="/opt/ext/ffmpeg/ffmpeg-n5.0.3/lib"

# =============================== 下载源码
DOWNLOAD_SRC_SH="$SCRIPT_DIR/../download_extract_user.sh"

## 源码的解压目录
SOURCE_TARGET_DIR=$HOME/whl_build/audio

## 下载源码
if [ ! -d "$SOURCE_TARGET_DIR" ]; then
    $DOWNLOAD_SRC_SH "https://archive.spacemit.com/ros2/prebuilt/source_code/audio.tar.gz" $SOURCE_TARGET_DIR
else
...
for version in "${TO_BUILD_VERSIONS[@]}"; do
    ...
    ## 切换到指定版本
    echo "📌 切换到 torchaudio v$BUILD_VERSION ..."
    ...
    echo "✅ 初始化完成，准备开始构建 torchaudio v$BUILD_VERSION"
    ...
    if [ -d "$SOURCE_TARGET_DIR" ]; then
        ...
        echo "Error no torchaudio project dir !"
        exit 1
    fi
