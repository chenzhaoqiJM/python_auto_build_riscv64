...
# =============================== 需要构建的版本
TO_BUILD_VERSIONS=("0.23.0" "0.22.1" "0.22.0" "0.21.0" "0.20.1" "0.20.0" "0.19.1" "0.19.0")

# =============================== 构建变量
export TORCHVISION_BUILD_NUMBER=0
export PKG_NAME="torchvision"

# =============================== 下载源码
DOWNLOAD_SRC_SH="$SCRIPT_DIR/../download_extract_user.sh"

## 源码解压路径
SOURCE_TARGET_DIR=$HOME/whl_build/vision

## 下载并解压源码
if [ ! -d "$SOURCE_TARGET_DIR" ]; then
    $DOWNLOAD_SRC_SH "https://archive.spacemit.com/ros2/prebuilt/source_code/vision.tar.gz" $SOURCE_TARGET_DIR
else
...
ADD_NUMPY_SCRIPT="$SCRIPT_DIR/04add_numpy.py"
...
for version in "${TO_BUILD_VERSIONS[@]}"; do
    ...
    ## 比较 v < 0.20.0，低版本需要开启这个环境变量
    if [ "$(printf '%s\n%s\n' "$version" "0.20.0" | sort -V | head -n1)" = "$version" ] && [ "$version" != "0.20.0" ]; then
    ...
    ## 切换到指定版本
    echo "📌 切换到 torchvision v$BUILD_VERSION ..."
    if ! git checkout -f "v$BUILD_VERSION"; then
        echo "❌ ERROR: 切换到版本 v$BUILD_VERSION 失败"
        exit 1
    fi
    ...
    echo "✅ 初始化完成，准备开始构建 $PKG_NAME v$BUILD_VERSION"
