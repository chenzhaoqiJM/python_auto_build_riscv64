...
# 可配置字段
export PIP_CACHE_DIR="$HOME/.cache/pip/build_from_src_keep_uv_$BUILD_FOR_VERSION"
export WHEELS_REPAIR_DIR="$HOME/.mywheel_repair/build_from_src_keep_uv_$BUILD_FOR_VERSION"

BUILD_TMPDIR="$HOME/.mytmp/build_from_src_keep_uv_$BUILD_FOR_VERSION"
VENV_NAME="build_from_src_keep_uv_$BUILD_FOR_VERSION"
VENV_DIR="$HOME/pyenvs/$VENV_NAME"
DIST_DIR="$HOME/pyenvs/store"
WHEEL_CACHE_DIR="$HOME/.mywheels/build_from_src_keep_uv_$BUILD_FOR_VERSION"

mkdir -p "$BUILD_TMPDIR" "$WHEEL_CACHE_DIR" "$DIST_DIR"

echo "!!! 临时文件夹在：$BUILD_TMPDIR ; 编译完成后请手动删除它..........."
...
if [ -d "$DIST_DIR/$VENV_NAME" ]; then
    ...
    pip install numpy maturin scipy pyelftools pybind11 Cython
...
for PACKAGE_SOURCE_PATH in "$@"; do
    ...
    # 动态环境变量
    source "$ENV_LOADER_SH" "$PACKAGE_SOURCE_PATH"
    load_env
    echo "LD_LIBRARY_PATH=${LD_LIBRARY_PATH}"
    ...
    build_generic_package() {
        ...
        cd "$PACKAGE_SOURCE_PATH" || { echo "❌ ERROR: 无法进入目录 $PACKAGE_SOURCE_PATH"; exit 1; }
