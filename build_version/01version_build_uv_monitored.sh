#!/bin/bash
# 01version_build_uv_monitored.sh - 带监控上报的版本更新构建脚本
# 这是 01version_build_uv.sh 的复制版本，集成了构建状态监控上报功能
set -e
...
# ========== 监控日志目录 ==========
BUILD_LOG_DIR="$HOME/.build_logs/version_uv_$BUILD_FOR_VERSION"
...
while true; do
    ...
    while IFS= read -r PACKAGE_NAME || [[ -n "$PACKAGE_NAME" ]]; do
        ...
        while IFS= read -r PACKAGE_VERSION_NAME || [[ -n "$PACKAGE_VERSION_NAME" ]]; do
            ...
            # ========== 监控：上报构建开始 ==========
            CURRENT_LOG_FILE="$BUILD_LOG_DIR/${PACKAGE_NAME//[^a-zA-Z0-9_-]/_}_${PACKAGE_VERSION_NAME//[^a-zA-Z0-9._-]/_}.log"
            if [ "$MONITOR_ENABLED" -eq 1 ]; then
                report_start "$PACKAGE_WITH_VERSION" "$SCRIPT_NAME"
